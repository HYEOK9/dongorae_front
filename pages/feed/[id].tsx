...
const FeedDetail = () => {
    ...
    async function fetchFeedDetail() {
        ...
        axios
            ...
            .then((res) => {
                ...
                hashTags =
                    res.data?.hashTags?.map((tag: string) =>
    ...
    return (
        <>
            {isLoading && (
                <Loading loadingMsg="피드 정보를 가져오는 중입니다" />
            )}
            <MainContainer>
                <FeedContainer
                    ...
                    <ContentsContainer>
                        ...
                        <div style={{ width: "calc(65% - 20px)" }}>
                            {!isEmpty(feedDetail?.photoUrls) && (
                                <ImgContainer>
                                    {feedDetail?.photoUrls?.map(
                                        (url: string, idx: number) => (
                                            <ImgHolder src={url} key={idx} />
                                        )
                                    )}
                                </ImgContainer>
                            )}
                        ...
                        <div style={{ width: "35%" }}>
                            ...
                            >
                                <div>
                                    <span
                                        style={{
                                            color: themeColorset.pointColor,
                                        }}
                                    >
                                        {location?.placeName}
                                    </span>
